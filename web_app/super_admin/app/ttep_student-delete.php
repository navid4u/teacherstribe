...
if(isset($_POST["id_ttep_student"]) && !empty($_POST["id_ttep_student"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['ttep_student']['columns'])) {
        foreach ($tables_and_columns_names['ttep_student']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            $sql = "SELECT `" . $columnName . "`
                    FROM `ttep_student`
                    WHERE `id_ttep_student` = ?";

            if ($stmt = mysqli_prepare($link, $sql)) {
                // Set parameters
                $param_id = trim($_POST["id_ttep_student"]);
    ...
    $sql = "DELETE FROM `ttep_student` WHERE `id_ttep_student` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: ttep_student-index.php");
        }
    }
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?id_ttep_student=" . $_GET["id_ttep_student"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="id_ttep_student" value="<?php echo trim($_GET["id_ttep_student"]); ?>"/>
