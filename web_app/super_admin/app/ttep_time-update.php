...
if(isset($_POST["ttep_time"]) && !empty($_POST["ttep_time"])){
    // Get hidden input value
    $ttep_time = $_POST["ttep_time"];
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $id_ttep_teacher = $_POST["id_ttep_teacher"] == "" ? null : trim($_POST["id_ttep_teacher"]);
		$time_description = $_POST["time_description"] == "" ? null : trim($_POST["time_description"]);
		

        // Prepare an update statement

        $stmt = $link->prepare("UPDATE `ttep_time` SET `id_ttep_teacher`=?,`time_description`=? WHERE `ttep_time`=?");

        try {
            $stmt->execute([ $id_ttep_teacher, $time_description, $ttep_time  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: ttep_time-read.php?ttep_time=$ttep_time");
        } else {
...
$_GET["ttep_time"] = trim($_GET["ttep_time"]);
if(isset($_GET["ttep_time"]) && !empty($_GET["ttep_time"])){
    // Get URL parameter
    $ttep_time =  trim($_GET["ttep_time"]);

    // Prepare a select statement
    $sql = "SELECT * FROM `ttep_time` WHERE `ttep_time` = ?";
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $id_ttep_teacher = htmlspecialchars($row["id_ttep_teacher"] ?? "");
					$time_description = htmlspecialchars($row["time_description"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                                            <label for="id_ttep_teacher">id_ttep_teacher</label>
                                            <input type="number" name="id_ttep_teacher" id="id_ttep_teacher" class="form-control" value="<?php echo @$id_ttep_teacher; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="time_description">time_description</label>
                                            <input type="text" name="time_description" id="time_description" maxlength="45" class="form-control" value="<?php echo @$time_description; ?>">
                                        </div>

                        <input type="hidden" name="ttep_time" value="<?php echo $ttep_time; ?>"/>
                        <p>
                        ...
                        <hr>
                        <p>
                            <a href="ttep_time-read.php?ttep_time=<?php echo $_GET["ttep_time"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
                            <a href="ttep_time-delete.php?ttep_time=<?php echo $_GET["ttep_time"];?>" class="btn btn-danger"><?php translate('Delete Record') ?></a>
                            <a href="ttep_time-index.php" class="btn btn-primary"><?php translate('Back to List') ?></a>
                        </p>
