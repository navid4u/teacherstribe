...
if(isset($_POST["id_ttep_announcement"]) && !empty($_POST["id_ttep_announcement"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['ttep_announcement']['columns'])) {
        foreach ($tables_and_columns_names['ttep_announcement']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            $sql = "SELECT `" . $columnName . "`
                    ...
                    WHERE `id_ttep_announcement` = ?";

            if ($stmt = mysqli_prepare($link, $sql)) {
                // Set parameters
                $param_id = trim($_POST["id_ttep_announcement"]);
    ...
    $sql = "DELETE FROM `ttep_announcement` WHERE `id_ttep_announcement` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: ttep_announcement-index.php");
        }
    }
    ...
    if(empty($_GET["id_ttep_announcement"])){
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?id_ttep_announcement=" . $_GET["id_ttep_announcement"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="id_ttep_announcement" value="<?php echo trim($_GET["id_ttep_announcement"]); ?>"/>
