...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $link->prepare("INSERT INTO `ttep_project` (`title`, `detail`, `ttep_teacher_username`, `time`, `importance`, `required`, `progress`, `extra_information`, `short_long`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");

        try {
            $stmt->execute([ $title, $detail, $ttep_teacher_username, $time, $importance, $required, $progress, $extra_information, $short_long ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            ...
            header("location: ttep_project-read.php?id_ttep_projects=$new_id");
        } else {
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <a href="ttep_project-index.php" class="btn btn-secondary"><?php translate('Cancel') ?></a>
                    </form>
