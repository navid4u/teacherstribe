...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$eventscol = $_POST["eventscol"] == "" ? null : trim($_POST["eventscol"]);
		$start_date = trim($_POST["start_date"]);
		$end_date = trim($_POST["end_date"]);
		$posted_by = $_POST["posted_by"] == "" ? null : trim($_POST["posted_by"]);
		

        // Prepare an update statement

        $stmt = $link->prepare("UPDATE `events` SET `title`=?,`description`=?,`eventscol`=?,`start_date`=?,`end_date`=?,`posted_by`=? WHERE `id`=?");

        try {
            $stmt->execute([ $title, $description, $eventscol, $start_date, $end_date, $posted_by, $id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: events-read.php?id=$id");
        } else {
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $title = htmlspecialchars($row["title"] ?? "");
					...
					$eventscol = htmlspecialchars($row["eventscol"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                                            ...
                                            <input type="text" name="title" id="title" maxlength="190" class="form-control" value="<?php echo @$title; ?>">
                                        </div>
						<div class="form-group">
						...
						<div class="form-group">
                                            <label for="eventscol">eventscol</label>
                                            <input type="text" name="eventscol" id="eventscol" maxlength="45" class="form-control" value="<?php echo @$eventscol; ?>">
                                        </div>
						<div class="form-group">
						...
						<div class="form-group">
                                            ...
                                            <input type="text" name="posted_by" id="posted_by" maxlength="45" class="form-control" value="<?php echo @$posted_by; ?>">
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="events-read.php?id=<?php echo $_GET["id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
                            <a href="events-delete.php?id=<?php echo $_GET["id"];?>" class="btn btn-danger"><?php translate('Delete Record') ?></a>
                            <a href="events-index.php" class="btn btn-primary"><?php translate('Back to List') ?></a>
                        </p>
