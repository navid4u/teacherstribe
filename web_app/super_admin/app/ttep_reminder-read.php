...
if(isset($_GET["id_ttep_reminder"]) && !empty($_GET["id_ttep_reminder"])){
    ...
    $sql = "SELECT `ttep_reminder`.* 
            FROM `ttep_reminder` 
            WHERE `ttep_reminder`.`id_ttep_reminder` = ?
            GROUP BY `ttep_reminder`.`id_ttep_reminder`;";

    if($stmt = mysqli_prepare($link, $sql)){
        // Set parameters
        $param_id = trim($_GET["id_ttep_reminder"]);
                ...
                <div class="col-md-8 mx-auto">
                    <div class="page-header">
                        <h1><?php translate('View Record') ?></h1>
									...
									// Check if the column is file upload
									// echo '<pre>';
									// print_r($tables_and_columns_names['ttep_reminder']["columns"]['date']);
									// echo '</pre>';
									$has_link_file = isset($tables_and_columns_names['ttep_reminder']["columns"]['date']['is_file']) ? true : false;
									if ($has_link_file){
									    $is_file = $tables_and_columns_names['ttep_reminder']["columns"]['date']['is_file'];
									...
									<div class="form-group">
									    <h4>done</h4>
									    <?php if ($has_link_file): ?>
									        <p class="form-control-static"><?php echo $link_file ?><?php echo htmlspecialchars($row["done"] ?? ""); ?><?php echo $end_link_file ?></p>
									    <?php endif ?>
									</div>									<?php
									// Check if the column is file upload
									// echo '<pre>';
                    ...
                    <hr>
                    <p>
                        <a href="ttep_reminder-update.php?id_ttep_reminder=<?php echo $_GET["id_ttep_reminder"];?>" class="btn btn-warning"><?php translate('Update Record') ?></a>
                        ...
                        <a href="ttep_reminder-create.php" class="btn btn-success"><?php translate('Add New Record') ?></a>
