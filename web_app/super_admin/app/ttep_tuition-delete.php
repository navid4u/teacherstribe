...
if(isset($_POST["id_ttep_payment"]) && !empty($_POST["id_ttep_payment"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['ttep_tuition']['columns'])) {
        foreach ($tables_and_columns_names['ttep_tuition']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            $sql = "SELECT `" . $columnName . "`
                    FROM `ttep_tuition`
                    WHERE `id_ttep_payment` = ?";

            if ($stmt = mysqli_prepare($link, $sql)) {
                // Set parameters
                $param_id = trim($_POST["id_ttep_payment"]);
    ...
    $sql = "DELETE FROM `ttep_tuition` WHERE `id_ttep_payment` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: ttep_tuition-index.php");
        }
    }
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?id_ttep_payment=" . $_GET["id_ttep_payment"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="id_ttep_payment" value="<?php echo trim($_GET["id_ttep_payment"]); ?>"/>
