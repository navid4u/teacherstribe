...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $link->prepare("INSERT INTO `ttep_announcement` (`ttep_teacher_username`, `ttep_student_username`, `description`, `date`, `title`, `all_s`) VALUES (?, ?, ?, ?, ?, ?)");

        try {
            $stmt->execute([ $ttep_teacher_username, $ttep_student_username, $description, $date, $title, $all_s ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            ...
            header("location: ttep_announcement-read.php?id_ttep_announcement=$new_id");
        } else {
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <a href="ttep_announcement-index.php" class="btn btn-secondary"><?php translate('Cancel') ?></a>
                    </form>
