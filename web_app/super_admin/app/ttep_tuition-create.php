...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $link->prepare("INSERT INTO `ttep_tuition` (`ttep_teacher_username`, `id_ttep_student`, `date`, `extra_information`, `amount`, `image_path`, `sessioncount`, `reason`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

        try {
            $stmt->execute([ $ttep_teacher_username, $id_ttep_student, $date, $extra_information, $amount, $image_path, $sessioncount, $reason ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            ...
            header("location: ttep_tuition-read.php?id_ttep_payment=$new_id");
        } else {
...
<?php endif ?>
                        ...
                        <a href="ttep_tuition-index.php" class="btn btn-secondary"><?php translate('Cancel') ?></a>
                    </form>
