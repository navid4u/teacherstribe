...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $link->prepare("UPDATE `posts` SET `title`=?,`description`=?,`slug`=?,`posted_by`=?,`date`=?,`username`=?,`privacy`=? WHERE `id`=?");

        try {
            $stmt->execute([ $title, $description, $slug, $posted_by, $date, $username, $privacy, $id  ]);
        } catch (Exception $e) {
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $title = htmlspecialchars($row["title"] ?? "");
					...
					$privacy = htmlspecialchars($row["privacy"] ?? "");
