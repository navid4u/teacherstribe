...
if(isset($_POST["id_ttep_projects"]) && !empty($_POST["id_ttep_projects"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['ttep_project']['columns'])) {
        foreach ($tables_and_columns_names['ttep_project']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            $sql = "SELECT `" . $columnName . "`
                    FROM `ttep_project`
                    WHERE `id_ttep_projects` = ?";

            if ($stmt = mysqli_prepare($link, $sql)) {
                // Set parameters
                $param_id = trim($_POST["id_ttep_projects"]);
    ...
    $sql = "DELETE FROM `ttep_project` WHERE `id_ttep_projects` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: ttep_project-index.php");
        }
    }
    ...
    if(empty($_GET["id_ttep_projects"])){
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?id_ttep_projects=" . $_GET["id_ttep_projects"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="id_ttep_projects" value="<?php echo trim($_GET["id_ttep_projects"]); ?>"/>
