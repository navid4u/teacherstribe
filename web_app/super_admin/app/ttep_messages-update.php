...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $receiver = $_POST["receiver"] == "" ? null : trim($_POST["receiver"]);
		$sender = $_POST["sender"] == "" ? null : trim($_POST["sender"]);
		$message = $_POST["message"] == "" ? null : trim($_POST["message"]);
		$readstatus = trim($_POST["readstatus"]);
		

        // Prepare an update statement

        $stmt = $link->prepare("UPDATE `ttep_messages` SET `receiver`=?,`sender`=?,`message`=?,`readstatus`=? WHERE `id`=?");

        try {
            $stmt->execute([ $receiver, $sender, $message, $readstatus, $id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: ttep_messages-read.php?id=$id");
        } else {
...
if(isset($_GET["id"]) && !empty($_GET["id"])){
    ...
    $sql = "SELECT * FROM `ttep_messages` WHERE `id` = ?";
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $receiver = htmlspecialchars($row["receiver"] ?? "");
					...
					$readstatus = htmlspecialchars($row["readstatus"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                                            ...
                                            <input type="text" name="receiver" id="receiver" maxlength="255" class="form-control" value="<?php echo @$receiver; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="sender">sender</label>
                                            <input type="text" name="sender" id="sender" maxlength="255" class="form-control" value="<?php echo @$sender; ?>">
                                        </div>
						<div class="form-group">
						...
						<div class="form-group">
                                            <label for="readstatus">readstatus*</label>
                                            <input type="text" name="readstatus" id="readstatus" maxlength="11" class="form-control" value="<?php echo @$readstatus; ?>">
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="ttep_messages-read.php?id=<?php echo $_GET["id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
