...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$id_get = trim($_POST["id_get"]);
		$plan = $_POST["plan"] == "" ? null : trim($_POST["plan"]);
		

        // Prepare an update statement

        $stmt = $link->prepare("UPDATE `ttep_premium` SET `username`=?,`trans_id`=?,`id_get`=?,`plan`=? WHERE `id`=?");

        try {
            $stmt->execute([ $username, $trans_id, $id_get, $plan, $id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: ttep_premium-read.php?id=$id");
        } else {
...
if(isset($_GET["id"]) && !empty($_GET["id"])){
    ...
    $sql = "SELECT * FROM `ttep_premium` WHERE `id` = ?";
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $username = htmlspecialchars($row["username"] ?? "");
					...
					$id_get = htmlspecialchars($row["id_get"] ?? "");
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
                                            ...
                                            <input type="number" name="trans_id" id="trans_id" class="form-control" value="<?php echo @$trans_id; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="id_get">id_get*</label>
                                            <input type="number" name="id_get" id="id_get" class="form-control" value="<?php echo @$id_get; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="plan">plan</label>
                                            <input type="text" name="plan" id="plan" maxlength="50" class="form-control" value="<?php echo @$plan; ?>">
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="ttep_premium-read.php?id=<?php echo $_GET["id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
