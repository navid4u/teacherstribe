...
$columns = array('id', 'title', 'description', 'slug', 'posted_by', 'date', 'username', 'privacy');
...
if (!empty($_GET['search'])) {
    ...
    if (strpos('`posts`.`id`, `posts`.`title`, `posts`.`description`, `posts`.`slug`, `posts`.`posted_by`, `posts`.`date`, `posts`.`username`, `posts`.`privacy`', ',')) {
        $where_statement .= " AND CONCAT_WS (`posts`.`id`, `posts`.`title`, `posts`.`description`, `posts`.`slug`, `posts`.`posted_by`, `posts`.`date`, `posts`.`username`, `posts`.`privacy`) LIKE '%$search%'";
    } else {
        $where_statement .= " AND `posts`.`id`, `posts`.`title`, `posts`.`description`, `posts`.`slug`, `posts`.`posted_by`, `posts`.`date`, `posts`.`username`, `posts`.`privacy` LIKE '%$search%'";
    }
...
$group_clause = !empty($order) && $order == 'id' ? "GROUP BY `posts`.`$order`" : '';
...
if ($isCsvExport) {
    ...
    exportAsCSV($data, $db_name, $tables_and_columns_names, 'posts', $link, false);
    exit;
}
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="posts-create.php" class="btn btn-success float-right"><?php translate('Add New Record') ?></a>
                        <a href="posts-index.php" class="btn btn-info float-right mr-2"><?php translate('Reset View') ?></a>
                        <a href="posts-index.php?export=csv" class="btn btn-primary float-right mr-2"><?php translate('Export as CSV') ?></a>
                        ...
                        if($result) :
                            if(mysqli_num_rows($result) > 0) :
                                ...
                                <table class='table table-bordered table-striped'>
									...
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=slug&sort=".$sort_link.">slug</a></th>";
									...
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "asc" ? "desc" : "asc";
									$sort_link = isset($_GET["order"]) && $_GET["order"] == $columnname && $_GET["sort"] == "desc" ? "asc" : $sort_link;
									echo "<th><a href=?search=$search&order=privacy&sort=".$sort_link.">privacy</a></th>";
 ?>
                                    ...
                                    <tbody>
                                        <?php while($row = mysqli_fetch_array($result)): ?>
											...
											echo nl2br(htmlspecialchars($row['slug'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t";											echo "<td>";
											...
											echo nl2br(htmlspecialchars($row['privacy'] ?? ""));
											if ($has_link_file){
											    echo $is_file ? "</a>" : "";
											}
											echo "</td>"."\n\t\t\t\t\t\t\t\t\t\t\t\t"; ?>
                                                <td>
                                                    ...
                                                    $column_id = 'id';
                                                    if (!empty($column_id)): ?>
                                                        <a id='read-<?php echo $row['id']; ?>' href='posts-read.php?id=<?php echo $row['id']; ?>' title='<?php echo addslashes(translate('View Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-info'><i class='far fa-eye'></i></a>
                                                        <a id='update-<?php echo $row['id']; ?>' href='posts-update.php?id=<?php echo $row['id']; ?>' title='<?php echo addslashes(translate('Update Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-warning'><i class='far fa-edit'></i></a>
                                                        <a id='delete-<?php echo $row['id']; ?>' href='posts-delete.php?id=<?php echo $row['id']; ?>' title='<?php echo addslashes(translate('Delete Record', false)); ?>' data-toggle='tooltip' class='btn btn-sm btn-danger'><i class='far fa-trash-alt'></i></a>
                                                    <?php else: ?>
