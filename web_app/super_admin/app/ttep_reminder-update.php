...
if(isset($_POST["id_ttep_reminder"]) && !empty($_POST["id_ttep_reminder"])){
    // Get hidden input value
    $id_ttep_reminder = $_POST["id_ttep_reminder"];
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
		...
		$done = $_POST["done"] == "" ? null : trim($_POST["done"]);
		$detail = $_POST["detail"] == "" ? null : trim($_POST["detail"]);
		

        // Prepare an update statement

        $stmt = $link->prepare("UPDATE `ttep_reminder` SET `ttep_teacher_username`=?,`date`=?,`title`=?,`done`=?,`detail`=? WHERE `id_ttep_reminder`=?");

        try {
            $stmt->execute([ $ttep_teacher_username, $date, $title, $done, $detail, $id_ttep_reminder  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: ttep_reminder-read.php?id_ttep_reminder=$id_ttep_reminder");
        } else {
...
$_GET["id_ttep_reminder"] = trim($_GET["id_ttep_reminder"]);
if(isset($_GET["id_ttep_reminder"]) && !empty($_GET["id_ttep_reminder"])){
    // Get URL parameter
    $id_ttep_reminder =  trim($_GET["id_ttep_reminder"]);

    // Prepare a select statement
    $sql = "SELECT * FROM `ttep_reminder` WHERE `id_ttep_reminder` = ?";
    if($stmt = mysqli_prepare($link, $sql)){
        // Set parameters
        $param_id = $id_ttep_reminder;
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
                                            ...
                                            <input type="text" name="done" id="done" maxlength="45" class="form-control" value="<?php echo @$done; ?>">
                                        </div>
						<div class="form-group">
                        ...
                        <input type="hidden" name="id_ttep_reminder" value="<?php echo $id_ttep_reminder; ?>"/>
                        <p>
                        ...
                        <hr>
                        <p>
                            <a href="ttep_reminder-read.php?id_ttep_reminder=<?php echo $_GET["id_ttep_reminder"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
                            <a href="ttep_reminder-delete.php?id_ttep_reminder=<?php echo $_GET["id_ttep_reminder"];?>" class="btn btn-danger"><?php translate('Delete Record') ?></a>
                            <a href="ttep_reminder-index.php" class="btn btn-primary"><?php translate('Back to List') ?></a>
                        </p>
