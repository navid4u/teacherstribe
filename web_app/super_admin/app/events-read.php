        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-8 mx-auto">
                    <div class="page-header">
                        <h1><?php translate('View Record') ?></h1>
									...
									<div class="form-group">
									    <h4>eventscol</h4>
									    <?php if ($has_link_file): ?>
									        <p class="form-control-static"><?php echo $link_file ?><?php echo htmlspecialchars($row["eventscol"] ?? ""); ?><?php echo $end_link_file ?></p>
									    <?php endif ?>
									</div>									<?php
									// Check if the column is file upload
									// echo '<pre>';
									// print_r($tables_and_columns_names['events']["columns"]['start_date']);
									// echo '</pre>';
									$has_link_file = isset($tables_and_columns_names['events']["columns"]['start_date']['is_file']) ? true : false;
									if ($has_link_file){
									    $is_file = $tables_and_columns_names['events']["columns"]['start_date']['is_file'];
									...
									// Check if the column is file upload
									// echo '<pre>';
									// print_r($tables_and_columns_names['events']["columns"]['end_date']);
									// echo '</pre>';
									$has_link_file = isset($tables_and_columns_names['events']["columns"]['end_date']['is_file']) ? true : false;
									if ($has_link_file){
									    $is_file = $tables_and_columns_names['events']["columns"]['end_date']['is_file'];
                    ...
                    <hr>
                    <p>
                        <a href="events-update.php?id=<?php echo $_GET["id"];?>" class="btn btn-warning"><?php translate('Update Record') ?></a>
                        <a href="events-delete.php?id=<?php echo $_GET["id"];?>" class="btn btn-danger"><?php translate('Delete Record') ?></a>
                        <a href="events-create.php" class="btn btn-success"><?php translate('Add New Record') ?></a>
