...
if(isset($_POST["id_ttep_teacher"]) && !empty($_POST["id_ttep_teacher"])){
    ...
    $fileColumns = [];
    if (isset($tables_and_columns_names['ttep_teacher']['columns'])) {
        foreach ($tables_and_columns_names['ttep_teacher']['columns'] as $columnName => $columnDetails) {
    ...
    if (!empty($fileColumns)) {
        foreach ($fileColumns as $columnName) {
            ...
            $sql = "SELECT `" . $columnName . "`
                    FROM `ttep_teacher`
                    WHERE `id_ttep_teacher` = ?";

            if ($stmt = mysqli_prepare($link, $sql)) {
                // Set parameters
                $param_id = trim($_POST["id_ttep_teacher"]);
    ...
    $sql = "DELETE FROM `ttep_teacher` WHERE `id_ttep_teacher` = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if (!isset($error)){
            ...
            header("location: ttep_teacher-index.php");
        }
    }
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . "?id_ttep_teacher=" . $_GET["id_ttep_teacher"]; ?>" method="post">
                    <?php print_error_if_exists(@$error); ?>
                        <div class="alert alert-danger fade-in">
                            <input type="hidden" name="id_ttep_teacher" value="<?php echo trim($_GET["id_ttep_teacher"]); ?>"/>
